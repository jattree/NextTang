...
usage() {
    printf '%s\n' 'usage: boards/console138k/build_cpu.sh --toolchain vendor --profile release --output ABSOLUTE_DIRECTORY'
}
...
while (($#)); do
    case "$1" in
        --toolchain|--profile|--output)
            ...
            ;;
        -h|--help) usage; exit 0 ;;
        *) printf 'console138k CPU build: unknown argument: %s\n' "$1" >&2; usage >&2; exit 2 ;;
    esac
done

if [[ "$toolchain" != vendor ]]; then
    printf 'console138k CPU build: toolchain not implemented: %s\n' "$toolchain" >&2
    exit 2
fi
if [[ "$profile" != release ]]; then
    printf 'console138k CPU build: profile not implemented: %s\n' "$profile" >&2
    exit 2
fi
if [[ "$output_dir" != /* ]]; then
    printf '%s\n' 'console138k CPU build: --output must be an absolute path' >&2
    exit 2
fi
if [[ ! -d "$output_dir" ]]; then
    printf 'console138k CPU build: output directory does not exist: %s\n' "$output_dir" >&2
    exit 2
fi
if [[ -n $(find "$output_dir" -mindepth 1 -maxdepth 1 -print -quit) ]]; then
    printf 'console138k CPU build: output directory is not empty: %s\n' "$output_dir" >&2
    exit 2
fi

base_name=nexttang_console138k_cpu
...
source_files=(
    ...
    "$repo_root/boards/console138k/nexttang_console138k_cpu.v"
    "$repo_root/boards/console138k/console138k_cpu.cst"
    "$repo_root/boards/console138k/console138k_cpu.sdc"
)
...
(
    ...
    printf 'console138k CPU build: FAIL (see %s)\n' "$build_log" >&2
    tail -30 "$build_log" >&2
    exit 1
}
...
if [[ ! -f "$bitstream" ]]; then
    printf 'console138k CPU build: no bitstream produced (see %s)\n' "$build_log" >&2
    exit 1
fi

python3 "$repo_root/scripts/check_timing.py" "$timing_report" || exit 1

printf 'console138k CPU build: PASS\n'
printf 'bitstream: %s\n' "$bitstream"
