...
module usb_hid_host #(
  ...
  output wire        dbg_byte_strobe,
  output wire        dbg_packet_valid,
  output reg  [63:0] dbg_config_snapshot,
  output reg         dbg_config_snapshot_valid,
  output wire        dbg_full_speed,
  output wire [15:0] dbg_speed_sample,
...
ukp #(
  ...
  .connected(connected),
  .full_speed(full_speed),
  .dbg_speed_sample(dbg_speed_sample),
  .connerr(connerr),
  .busy(busy),
  .rom_addr(rom_addr),
  .rom_dout(rom_dout),
...
// The configuration descriptor is a byte stream, not an eight-byte record.
// Find its first interface descriptor by the standard 09/04 header rather
// than relying on where the final packet happens to leave those bytes in dat.
reg       config_scan_pending;
reg       config_scan_active;
reg       interface_found;
reg [3:0] interface_scan_position;
reg [1:0] config_scan_pending_count;
reg [7:0] config_scan_pending0;
reg [7:0] config_scan_pending1;
reg [7:0] interface_number;
reg [7:0] interface_alternate;
reg [7:0] interface_endpoints;
reg [7:0] interface_class;
reg [7:0] interface_subclass;
reg [7:0] interface_protocol;
reg [7:0] interface_string;
...
assign dbg_byte_strobe = ukpstb;
assign dbg_packet_valid = ukprdy;
assign dbg_full_speed = full_speed;
...
always @(posedge clk) begin
  if (reset || connerr) begin
    for (i = 0; i < 8; i = i + 1)
      regs[i] <= 8'b0;
    dbg_config_snapshot <= 64'b0;
    dbg_config_snapshot_valid <= 1'b0;
    config_scan_pending <= 1'b0;
    config_scan_active <= 1'b0;
    interface_found <= 1'b0;
    interface_scan_position <= 4'd0;
    config_scan_pending_count <= 2'd0;
    config_scan_pending0 <= 8'b0;
    config_scan_pending1 <= 8'b0;
    interface_number <= 8'b0;
    interface_alternate <= 8'b0;
    interface_endpoints <= 8'b0;
    interface_class <= 8'b0;
    interface_subclass <= 8'b0;
    interface_protocol <= 8'b0;
    interface_string <= 8'b0;

  end else begin
    dbg_config_snapshot_valid <= 1'b0;

    // The SAVE of product-ID high marks the end of the device-descriptor
    // extraction. The following START belongs to the configuration read.
    if (ukpstart) begin
      config_scan_active <= config_scan_pending && !interface_found;
      config_scan_pending <= 1'b0;
      interface_scan_position <= 4'd0;
      config_scan_pending_count <= 2'd0;
    end

    // ukpstb includes the two CRC16 bytes. Delay each packet by two bytes so
    // only committed payload reaches the descriptor scanner, then discard the
    // pending pair when ukprdy falls at the packet boundary.
    if (ukprdy_r && !ukprdy)
      config_scan_pending_count <= 2'd0;

    if (ukpstb && config_scan_active && !interface_found) begin
      if (config_scan_pending_count == 0) begin
        config_scan_pending0 <= ukpdat;
        config_scan_pending_count <= 2'd1;
      end else if (config_scan_pending_count == 1) begin
        config_scan_pending1 <= ukpdat;
        config_scan_pending_count <= 2'd2;
      end else begin
        config_scan_pending0 <= config_scan_pending1;
        config_scan_pending1 <= ukpdat;

        case (interface_scan_position)
          4'd0: begin
            if (config_scan_pending0 == 8'h09)
              interface_scan_position <= 4'd1;
          end
          4'd1: begin
            if (config_scan_pending0 == 8'h04)
              interface_scan_position <= 4'd2;
            else if (config_scan_pending0 != 8'h09)
              interface_scan_position <= 4'd0;
          end
          4'd2: begin
            interface_number <= config_scan_pending0;
            interface_scan_position <= 4'd3;
          end
          4'd3: begin
            interface_alternate <= config_scan_pending0;
            interface_scan_position <= 4'd4;
          end
          4'd4: begin
            interface_endpoints <= config_scan_pending0;
            interface_scan_position <= 4'd5;
          end
          4'd5: begin
            interface_class <= config_scan_pending0;
            interface_scan_position <= 4'd6;
          end
          4'd6: begin
            interface_subclass <= config_scan_pending0;
            interface_scan_position <= 4'd7;
          end
          default: begin
            interface_protocol <= config_scan_pending0;
            regs[4] <= interface_class;
            regs[5] <= interface_subclass;
            regs[6] <= config_scan_pending0;
            interface_found <= 1'b1;
            config_scan_active <= 1'b0;
            interface_scan_position <= 4'd8;
          end
        endcase
      end
    end
    ...
    if (save) begin
      // Retain the legacy fixed-slot extraction as a fallback when no valid
      // interface descriptor was found. Once the scanner succeeds, those
      // slots describe packet placement rather than descriptor fields.
      if (addra < 4 || addra > 6 || !interface_found)
        regs[addra[2:0]] <= dat[addrb[2:0]];

      if (addra == 3)
        config_scan_pending <= 1'b1;

      if (addra == 6) begin
        dbg_config_snapshot <= interface_found ? {
          interface_subclass, interface_class, interface_endpoints,
          interface_alternate, interface_number, 8'h04,
          interface_string, interface_protocol
        } : {
          dat[7], dat[6], dat[5], dat[4], dat[3], dat[2], dat[1], dat[0]
        };
        dbg_config_snapshot_valid <= 1'b1;
      end
    end
...
always @(*) begin
  ...
  if (KEYBOARD_SUPPORT &&
    ...
    casez ({x_input, vid, pid})
      ...
      {1'b0, 16'h0079, 16'h0006}: begin  // supplied DragonRise USB controller
        // Observed report layout:
        //   d[0]/d[1] primary X/Y axes, d[2] buttons 0..7,
        //   d[3]/d[4] secondary axes, d[5][3:0] hat and
        //   d[5][7:4] buttons 8..11.
        {game_l, game_r} = {dat[0][7:6] == 2'b00, dat[0][7:6] == 2'b11};
        {game_u, game_d} = {dat[1][7:6] == 2'b00, dat[1][7:6] == 2'b11};

        // Established SDL mapping: Y/B/A/X are buttons 0/1/2/3.
        {game_x, game_a, game_b, game_y} = dat[2][3:0];
        {game_sel, game_sta} = {dat[5][4], dat[5][5]};
        game_extra = dat[2][7:4];
      end
...
module ukp #(
  ...
  output reg  [15:0] dbg_speed_sample,
  output wire connerr,
  output wire busy
);
...
`elsif NEXTTANG_USB_SIM_FAST
wire interval_frame = interval == 600;
`else
...
// The 5x full-speed receiver samples after the three-tap filter has settled.
// Low speed has eight effective samples per bit and uses the midpoint retained
// by the original nand2mario/Sipeed 12 MHz host.
assign timing_rx = timing == (FULL_SPEED && full_speed ? 2 : 4) &&
                   prescaler == 0;
  ...
  if (reset || (&conct)) begin
    ...
    case (state)
      ...
      S_BX: begin
        case (inst)
          0: begin
            if (!cond && !connected && FULL_SPEED && !speed_detected &&
                dsum != 0) begin
              // Latch device speed once from the majority-filtered bus state.
              // A raw D+ sample here can relabel a full-speed device when a
              // single-cycle idle glitch occurs during enumeration.
              full_speed <= dsum > 0;
              speed_detected <= 1;
              dbg_speed_sample <= {
                dpi, dmi, dsum, eop, ug, cond, timing, dsum > 0
              };
            end
          end  // op=BE
          default: ;
