...
module nexttang_logo_framebuffer #(
    ...
    parameter integer BEAT_COUNT = 512,
    parameter integer PIXEL_ADDRESS_WIDTH = BEAT_ADDRESS_WIDTH + 5
) (
    input  wire        write_clock,
    input  wire        write_enable,
    input  wire        write_bank,
    input  wire [BEAT_ADDRESS_WIDTH-1:0] write_address,
    input  wire [255:0] write_data,
    input  wire        read_clock,
    input  wire        read_bank,
    input  wire [PIXEL_ADDRESS_WIDTH-1:0] read_address,
    ...
    generate
        for (lane = 0; lane < 32; lane = lane + 1) begin : framebuffer_lane
            reg [7:0] bank_zero [0:BEAT_COUNT-1];
            reg [7:0] bank_one [0:BEAT_COUNT-1];
            ...
            always @(posedge read_clock) begin
                zero_read_data <= bank_zero[
                    read_address[PIXEL_ADDRESS_WIDTH-1:5]
                ];
                one_read_data <= bank_one[
                    read_address[PIXEL_ADDRESS_WIDTH-1:5]
                ];
            end
